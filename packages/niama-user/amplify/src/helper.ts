...
export const getAddressFragmentsAsString = (addr: T.Address.Dto, fragments = addressFragments): string => ''; //fragments.reduce((r, f) => (addr[f] ? `${r !== '' ? `${r}|` : ''}${f}:${addr[f]!.short}:${addr[f]!.long}` : r), '');

export const getAttrsFromAddress = (addr: T.Address.Dto): T.InfoAttrsAddress => {
...
export function getAddressFragmentsDtoFromAttrs<Attrs extends T.InfoAttrsAddress>(attrs: Attrs): any {
  //T.Address.FragmentsDto {
