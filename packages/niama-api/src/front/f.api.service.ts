...
export function apiS<D = any>(options: N.ApiBootO<D> = {}) {
  return ({ app, Vue }) => {
    const defO: Required<N.ApiBootO<D>> = { initial: () => ({} as D), http: false, rest: false, resolvers: [], secured: false, ws: false };
    const o: Required<N.ApiBootO<D>> = { ...defO, ...options };
...
function getLink({ http, secured, ws, rest }: Required<N.ApiBootO>): ApolloLink {
  const restLink: N.Maybe<RestLink> = rest
    ? new RestLink({
  ...
  const httpLink: N.Maybe<HttpLink> = http ? new HttpLink({ credentials: 'include', uri: getUri('http', secured) }) : null;
  const wsLink: N.Maybe<WebSocketLink> = ws ? new WebSocketLink({ options: { reconnect: true }, uri: getUri('ws') }) : null;
