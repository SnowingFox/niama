...
import { authCapsFromGrants } from '@niama/auth';
import { Me } from '@niama/auth/back';
...
@Resolver('User')
export class UserRS<Role extends string, User extends UserB<Role>> {
  // LIFECYCLE =============================================================================================================================
  ...
  async getCanDelete(@Me() me: User, @Parent() item: N.UserOrm['Nexus']['Read']): Promise<Role[]> {
    return authCapsFromGrants(item, await this.userS.grants(me)).canDelete;
  }

  @ResolveProperty('canRead')
  async getCanRead(@Me() me: User, @Parent() item: N.UserOrm['Nexus']['Read']): Promise<Role[]> {
    return authCapsFromGrants(item, await this.userS.grants(me)).canRead;
  }

  @ResolveProperty('canUpdate')
  async getCanUpdate(@Me() me: User, @Parent() item: N.UserOrm['Nexus']['Read']): Promise<Role[]> {
    return authCapsFromGrants(item, await this.userS.grants(me)).canUpdate;
  }

  @ResolveProperty('canUpdateStatus')
  async getCanUpdateStatus(@Me() me: User, @Parent() item: N.UserOrm['Nexus']['Read']): Promise<Role[]> {
    return authCapsFromGrants(item, await this.userS.grants(me)).canUpdateStatus;
  }
  ...
  async readMany(
    @Me() me: NPri.User,
    @Args() args: N.OrmReadManyArgs<A.UserW, A.UserOB>
