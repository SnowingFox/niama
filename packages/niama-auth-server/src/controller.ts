import { Body, Controller, Delete, Get, HttpStatus, Post, Res, UnauthorizedException, UseGuards } from '@nestjs/common';
...
import { AuthCookieS } from './service.cookie';
...
export class AuthCT<Role extends string, User extends UserB<Role>> {
  ...
  async signUp(@Body() dto: AuthSignupI, @Res() res: Response) {
    let result: T.Maybe<User> = await this.userS.readResource({ username: dto.email });
